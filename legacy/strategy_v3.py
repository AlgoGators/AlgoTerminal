...
# --- seasonal adjustment --------------------------------------------------
# The crack spread has a strong calendar cycle (summer driving season pushes
# it up, winter pulls it down). A crush is defined against the crack's own
# usual level for this time of year, not against a raw rolling mean. The
# seasonal mean for each calendar month is an expanding mean of past
# observations in that same month (strictly before bar t), so it never
# leaks future data.
...
VOL_TARGET = 0.50     # annualized volatility target
...
def _seasonal_mean(spread: pd.Series) -> pd.Series:
    ...
    For each bar t, the seasonal estimate is the mean of all observations in
    the same calendar month that are strictly before t. This captures the
    crack's annual cycle without any lookahead. Bars with fewer than
    SEASON_MIN_OBS prior same-month observations get NaN.
    """
...
def generate_signals(prices: pd.Series) -> pd.Series:
    ...
    fade on the crack's z-score against its own seasonal norm: long when the
    crack is crushed well below its usual level for this time of year
    (z < -Z_ENTRY), exit when the crush fades back to Z_EXIT. The short side
    is optional and only fires in a rangebound regime.
    """
    spread, _ = _crack_spread(prices)

    # Deseasonalize, then z-score, using only data strictly before bar t.
    prev = spread.shift(1)
