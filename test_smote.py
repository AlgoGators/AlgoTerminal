...
for name, sampler in [
    ...
    ("BorderlineSMOTE", BorderlineSMOTE(random_state=7)),
    ("ADASYN", ADASYN(random_state=7)),
]:
    Xtr=feat.loc[train_idx, feature_cols]
    ytr=feat.loc[train_idx, "wind5"]
    if sampler is not None:
        try:
            Xres, yres = sampler.fit_resample(Xtr, ytr)
            print(f"{name} resampled {len(Xres)} pos {(yres==1).sum()} ({(yres==1).mean()*100:.1f}%)")
        ...
        X_train_res=Xtr
        y_train_res=ytr
    cat=CatBoostClassifier(depth=4, learning_rate=0.05, iterations=500, auto_class_weights='Balanced' if sampler is None else None, verbose=False, random_seed=7)
    # For resampled, no auto weight
    cat.fit(X_train_res, y_train_res, eval_set=(feat.loc[valid_idx, feature_cols], feat.loc[valid_idx,"wind5"]), verbose=False)
    for n, idx in [("valid",valid_idx),("test",test_idx),("hold",hold_idx)]:
        p=cat.predict_proba(feat.loc[idx, feature_cols])[:,1]
        y=feat.loc[idx,"wind5"]
        print(f" {name:16s} {n:5s} AUC {roc_auc_score(y,p):.3f} AP {average_precision_score(y,p):.3f}")
    print("---")
