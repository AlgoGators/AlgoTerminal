...
| 4 | Electrical load | Electricity demand shapes run rates and margins | Load or degree-day conditioning of crack legs | NYISO/PJM load or POWER-derived | medium | TESTED-kill-construction (batch2, degree-day proxy; EIA/NYISO constrained) |
...
| 6 | Increasing demand/supply | The model must not assume flat demand and supply | Product supplied (retry other EIA routes) + stock-change demand proxy drift | EIA API | medium | ANSWERED-partial (batch2: building tilt fails 2sd bar, weak retained); product supplied CONSTRAINED |
...
| 8 | Asymmetry mechanism | Why does the left tail revert and the right tail not | Phase 1/1R answered: tightness persists; only regime-conditional short carries info. Next: tightness-ending events (utilization pin + stock rebuild) as the short entry | EIA + panel | medium | ANSWERED (batch2: tightness-ending confirmations killed; regime filter stands) |
